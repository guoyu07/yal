...
/*
  ...
  +------------------------------------------------------------------------+
  | Authors: WenJun <wenjun1055@gmail.com>                                 |
  +------------------------------------------------------------------------+
*/
 ...
 */
void php_yal_init_globals(zend_yal_globals *yal_globals TSRMLS_DC) {

	/* Memory options */
	yal_globals->start_memory  = NULL;
	yal_globals->active_memory = NULL;

	/* Virtual Symbol Tables */
	yal_globals->active_symbol_table = NULL;

	/* Cache options */
	yal_globals->function_cache = NULL;

	/* Recursive Lock */
	yal_globals->recursive_lock = 0;

	/* Stats options */
	#ifndef PHALCON_RELEASE
	yal_globals->yal_stack_stats  = 0;
	yal_globals->yal_number_grows = 0;
	#endif

	/* ORM options*/
	yal_globals->orm.events 				  = 1;
	yal_globals->orm.virtual_foreign_keys 	  = 1;
	yal_globals->orm.column_renaming 		  = 1;
	yal_globals->orm.not_null_validations 	  = 1;
	yal_globals->orm.exception_on_failed_save = 0;
	yal_globals->orm.parser_cache             = NULL;

	/* DB options */
	yal_globals->db.escape_identifiers = 1;
}

/**
 * Initializes internal interface with extends
 */
zend_class_entry *yal_register_internal_interface_ex(zend_class_entry *orig_class_entry, char *parent_name TSRMLS_DC) {
 ...
 */
int yal_init_global(char *global, unsigned int global_length TSRMLS_DC) {
 ...
 */
int yal_get_global(zval **arr, char *global, unsigned int global_length TSRMLS_DC) {
 ...
 */
void yal_fast_count(zval *result, zval *value TSRMLS_DC) {
 ...
 */
int yal_fast_count_ev(zval *value TSRMLS_DC) {
 ...
 */
int yal_function_exists_ex(char *method_name, unsigned int method_len TSRMLS_DC) {
 ...
 */
int yal_function_quick_exists_ex(char *method_name, unsigned int method_len, unsigned long key TSRMLS_DC) {
 ...
 */
int yal_is_callable(zval *var TSRMLS_DC) {
 ...
 */
int yal_is_iterable(zval *arr, HashTable **arr_hash, HashPosition *hash_position, int duplicate, int reverse TSRMLS_DC) {
 ...
 */
void yal_inherit_not_found(char *class_name, char *inherit_name) {
	fprintf(stderr, "Yal Error: Class to extend '%s' was not found when registering class '%s'\n", class_name, inherit_name);
}

/**
 * Parses method parameters with minimum overhead
 */
int yal_fetch_parameters(int num_args TSRMLS_DC, int required_args, int optional_args, ...)
{
	va_list va;
