...
#define Y_RANDOM_ALNUM 0
#define Y_RANDOM_ALPHA 1
#define Y_RANDOM_HEXDEC 2
#define Y_RANDOM_NUMERIC 3
#define Y_RANDOM_NOZERO 4

/**
 * Fast call to php strlen
 */
void yal_fast_strlen(zval *return_value, zval *str){
 ...
 * Fast call to php strlen
 */
void yal_fast_strtolower(zval *return_value, zval *str){
 ...
 */
void yal_fast_join(zval *result, zval *glue, zval *pieces TSRMLS_DC){
 ...
 */
void yal_append_printable_zval(smart_str *implstr, zval **tmp TSRMLS_DC) {
 ...
 *
 */
void yal_fast_join_str(zval *return_value, char *glue, unsigned int glue_length, zval *pieces TSRMLS_DC){
	...
	while (zend_hash_get_current_data_ex(arr, (void **) &tmp, &pos) == SUCCESS) {
		yal_append_printable_zval(&implstr, tmp TSRMLS_CC);
		if (++i != numelems) {
 ...
 */
void yal_camelize(zval *return_value, zval *str TSRMLS_DC){
 ...
 * Convert dash/underscored texts returning camelized
 */
void yal_uncamelize(zval *return_value, zval *str TSRMLS_DC){
 ...
 */
void yal_fast_explode(zval *result, zval *delimiter, zval *str TSRMLS_DC){
 ...
 */
int yal_memnstr(zval *haystack, zval *needle TSRMLS_DC){
 ...
 * Check if a string is contained into another
 */
int yal_memnstr_str(zval *haystack, char *needle, unsigned int needle_length TSRMLS_DC){
 ...
 */
void yal_fast_strpos(zval *return_value, zval *haystack, zval *needle TSRMLS_DC){
 ...
 * Inmediate function resolution for strpos function
 */
void yal_fast_strpos_str(zval *return_value, zval *haystack, char *needle, unsigned int needle_length TSRMLS_DC){
 ...
 */
void yal_fast_stripos_str(zval *return_value, zval *haystack, char *needle, unsigned int needle_length TSRMLS_DC){
 ...
 */
void yal_fast_str_replace(zval *return_value, zval *search, zval *replace, zval *subject TSRMLS_DC){
 ...
 */
void yal_extract_named_params(zval *return_value, zval *str, zval *matches){
...
zval *yal_replace_marker(int named, zval *paths, zval *replacements, unsigned long *position, char *cursor, char *marker){
 ...
 */
void yal_replace_paths(zval *return_value, zval *pattern, zval *paths, zval *replacements TSRMLS_DC){
	...
	if (Z_TYPE_P(pattern) != IS_STRING || Z_TYPE_P(replacements) != IS_ARRAY || Z_TYPE_P(paths) != IS_ARRAY) {
		ZVAL_NULL(return_value);
		php_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid arguments supplied for yal_replace_paths()");
		return;
	}
		...
		if (parentheses_count == 0 && !looking_placeholder) {
			if (ch == '{') {
				...
				if (ch == '}') {
					...
					if (intermediate > 0) {
						if (bracket_count == 0) {
							replace = yal_replace_marker(1, paths, replacements, &position, cursor, marker);
							if (replace) {
								use_copy = 0;
								if (Z_TYPE_P(replace) != IS_STRING) {
		...
		if (bracket_count == 0 && !looking_placeholder) {
			if (ch == '(') {
				...
				if (ch == ')') {
					...
					if (intermediate > 0) {
						if (parentheses_count == 0) {
							replace = yal_replace_marker(0, paths, replacements, &position, cursor, marker);
							if (replace) {
								use_copy = 0;
								if (Z_TYPE_P(replace) != IS_STRING) {
									zend_make_printable_zval(replace, &replace_copy, &use_copy);
 ...
 */
int yal_start_with(zval *str, zval *compared, zval *ignore_case){
 ...
 */
int yal_start_with_str(zval *str, char *compared, unsigned int compared_length){
 ...
 */
int yal_start_with_str_str(char *str, unsigned int str_length, char *compared, unsigned int compared_length){
 ...
 */
int yal_end_with(zval *str, zval *compared, zval *ignore_case){
 ...
 */
int yal_end_with_str(zval *str, char *compared, unsigned int compared_length){
...
void yal_random_string(zval *return_value, zval *type, zval *length TSRMLS_DC){
	...
	if (Z_LVAL_P(type) > Y_RANDOM_NOZERO) {
		return;
	}
	...
	for (i = 0; i < Z_LVAL_P(length); i++) {
		...
		switch (Z_LVAL_P(type)) {
			...
			case Y_RANDOM_ALPHA:
				...
				break;
			case Y_RANDOM_HEXDEC:
				rand_type = (long) (php_mt_rand(TSRMLS_C) >> 1);
				RAND_RANGE(rand_type, 0, 1, PHP_MT_RAND_MAX);
				break;
			case Y_RANDOM_NUMERIC:
				rand_type = 0;
				break;
			case Y_RANDOM_NOZERO:
				rand_type = 5;
				break;
			default:
				continue;
 ...
 */
void yal_remove_extra_slashes(zval *return_value, zval *str){
 ...
 */
int yal_spprintf(char **message, int max_len, char *format, ...)
{
    va_list arg;
    int len;
...
void yal_substr(zval *return_value, zval *str, unsigned long from, unsigned long length TSRMLS_DC) {
	...
	if (Z_TYPE_P(str) != IS_STRING) {
		...
		php_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid arguments supplied for yal_substr()");
		RETURN_FALSE;
	}
...
void yal_append_printable_array(smart_str *implstr, zval *value TSRMLS_DC) {
		...
		while (zend_hash_get_current_data_ex(arr, (void **) &tmp, &pos) == SUCCESS) {
			...
			if (Z_TYPE_PP(tmp) == IS_OBJECT) {
				...
				if (Z_TYPE_PP(tmp) == IS_ARRAY) {
					yal_append_printable_array(implstr, *tmp TSRMLS_CC);
				} else {
					yal_append_printable_zval(implstr, tmp TSRMLS_CC);
				}
			}
 ...
 */
void yal_unique_key(zval *return_value, zval *prefix, zval *value TSRMLS_DC) {
	...
	if (Z_TYPE_P(value) == IS_ARRAY) {
		yal_append_printable_array(&implstr, value TSRMLS_CC);
	} else {
		yal_append_printable_zval(&implstr, &value TSRMLS_CC);
	}
...
void yal_preg_match(zval *return_value, zval *regex, zval *subject, zval *matches TSRMLS_DC)
{
	...
	if (Z_TYPE_P(regex) != IS_STRING) {
		php_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid arguments supplied for yal_preg_match()");
		RETURN_FALSE;
	}
